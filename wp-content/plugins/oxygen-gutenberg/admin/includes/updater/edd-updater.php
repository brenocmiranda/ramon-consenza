...
if( !class_exists( 'EDD_SL_Plugin_Updater_Oxygen_Gutenberg' ) ) {
	include( dirname( __FILE__ ) . '/EDD_SL_Plugin_Updater_Oxygen_Gutenberg.php' );
}
...
Class OxygenGutenbergPluginUpdater {
	...
	function init() {
		...
		// setup the updater
		$edd_updater = new EDD_SL_Plugin_Updater_Oxygen_Gutenberg( 
			$this->oxygen_url, 
			plugin_dir_path( dirname( dirname( dirname( __FILE__ ) ) ) ) . "oxygen-gutenberg.php", // main plugin file, specify for each add-on
			array( 
				'version' 	=> CT_OXYGEN_GUTENBERG_VERSION, 				// current version number
